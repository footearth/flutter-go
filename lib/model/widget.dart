...
import "package:flutter/material.dart" show
  WidgetBuilder
;
import 'package:flutter_go/utils/sql.dart' show
  Sql
;
...
class WidgetControlModel {
  ...
  // 获取Widget不同条件的列表
  Future<List<WidgetPoint>> getList(
    WidgetPoint widgetPoint
  ) async {
    List listJson = await sql.getByCondition(
      conditions: widgetPoint.toSqlCondition()
    )
    ;
    List<WidgetPoint> widgets =
      listJson.map(
        (json) => WidgetPoint.fromJSON(json)
      ).toList()
    ;
  ...
  // 通过name获取Cat对象信息
  Future<WidgetPoint> getCatByName(
    String name
  ) async {
    List json = await sql.getByCondition(
      conditions: {
        'name': name
      }
    )
  ...
  Future<List<WidgetPoint>> search(
    String name
  ) async {
    List json = await sql.search(
      conditions: {
        'name': name
      }
    )
